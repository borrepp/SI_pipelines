...
########################################################################################################################
def set_SItempdir_environ():
    # CONFIRM or ADD TEMPORAL DIR ('SI_PROCESSOR_TEMPDIR') into the variables of the environment
    if not os.environ.get('SI_PROCESSOR_TEMPDIR'):
        os.environ['SI_PROCESSOR_TEMPDIR'] = os.path.abspath(os.path.dirname(os.path.abspath(__file__)) +  '/SI_tempdir')
        print('SI_PROCESSOR_TEMPDIR variable was created')
	
    # CONFIRM THAT EXISTS or CREATE THE TEMPORAL DIR ('SI_PROCESSOR_TEMPDIR')
    if not os.path.exists(os.environ.get('SI_PROCESSOR_TEMPDIR')):
        os.mkdir(os.environ.get('SI_PROCESSOR_TEMPDIR'))
...
########################################################################################################################
def clear_SItempdir(verbose=True):
    tempdir_path = os.environ.get('SI_PROCESSOR_TEMPDIR')

    if tempdir_path is not None:
        # Clear tempDir
        for root, _, files in os.walk(tempdir_path, topdown=False):
            if root != tempdir_path:
                remove_dir = True
            else:
                remove_dir = False
...
def getProbeInfo(electrodeGroup_dataFrame, nwbFile):
    ...
    # Double check this electrodes were recorded with the same probe:
    ...
    return {'probeName': nameDevice, 'radius': diameterElec, 'radius_units': 'mm', 'num_shanks': num_shanks, 'shank_ids': shank_ids}
...
####################################################################################################################################################################################
def get_si_recording(parentRecordingFolder, sessionYear, sessionMonth, sessionDay, electrodeGroup_Name, session_index=None, localProcess_NWB=False, reset_tempdir=True):
    ...
    return si_recording, electrodeGroup_sessions
...
def plot_concatenations(si_recording_dict, plot_windows_secs=0.01, sampleChans=False, showPlots=True, savePlots=False, folderPlots=None):
    ...
    if nConcatenations>0:
        ...
        for c in range(nConcatenations):
            ...
            if savePlots: 
                _, sess_electrodeGroup_Name = os.path.split(si_recording.get_annotation('savePath'))
...
def plotPSD_randomChunks(si_recording, compare_CMR=True, plot_by_channel=True, chan_radius=None, showPlots=True, savePlots=False, folderPlots=None):
    ...
    if 'savePath' in si_recording._annotations:
        ...
        sess_electrodeGroup_Name = 'electrodeGroup'
    ...
    if compare_CMR and nChans>1:
        ...
        # Get Chunks of data
        data_chunk_cmr = si.get_random_data_chunks(
...
#########################################################################################################################################
def plot_peakLocations(si_recording, folderPeaks, extra_recording_label = '', peaks_options=None, locations_options=None, rewrite=False, locationsSubSampled=True,
    ...
    if savePlots or showPlots:
        ...
        ###############################################################################
        # Detect PEAKS 
        if not os.path.exists(os.path.join(folderPeaks, 'peaks_' + peaks_label + extra_recording_label + '.npy')) or rewrite:
            ...
            numpy.save(os.path.join(folderPeaks, 'peaks_' + peaks_label + extra_recording_label + '.npy'), peaks)
        ...
        if locationsSubSampled:
            ...
            if not os.path.exists(os.path.join(folderPeaks, 'peaksSampled_' + peaks_label + extra_recording_label + '.npy')) or rewrite:

                noise_levels = si.get_noise_levels(si_recording, return_scaled=False)

                peaks = numpy.load(os.path.join(folderPeaks, 'peaks_' + peaks_label + extra_recording_label + '.npy'))
                ...
                numpy.save(os.path.join(folderPeaks, 'peaksSampled_' + peaks_label + extra_recording_label + '.npy'), peaksSampled)
        ...
        peaks = numpy.load(os.path.join(folderPeaks, 'peaks' + peaksSampled_label + peaks_label + extra_recording_label + '.npy'))
        ...
        if nChans>1:
            ...
            # Localize PEAKS 
            ##################################################################
            if not os.path.exists(os.path.join(folderPeaks, 'peaks' + peaksSampled_label + peaks_label + '_locations' + loc_label + extra_recording_label + '.npy')) or rewrite:
                ...
                numpy.save(os.path.join(folderPeaks, 'peaks' + peaksSampled_label + peaks_label + '_locations' + loc_label + extra_recording_label + '.npy'), peaksSampled_locations)

                del peaksSampled_locations
                                
            peaks_locations = numpy.load(os.path.join(folderPeaks, 'peaks' + peaksSampled_label + peaks_label + '_locations' + loc_label + extra_recording_label + '.npy'))
        ...
        fig.suptitle(peaks_label + loc_label + extra_recording_label)

        if savePlots: 
            _, sess_electrodeGroup_Name = os.path.split(si_recording.get_annotation('savePath'))
            fig.set_figheight(fig_height_global)
            fig.set_figwidth(fig_width_global)
            fig.savefig(os.path.join(folderPlots, sess_electrodeGroup_Name + '_peaks' + peaksSampled_label + peaks_label + loc_label + extra_recording_label + '.eps'), dpi=fig_dpi, format='eps')
...
def plot_motion_outputs(peaks, peaks_locations, sampling_frequency, motion, extra_check, peaks_label, loc_label, motion_label, minY_loc, maxY_loc, 
        prefixRec = None, showPlots=True, savePlots=False, folderPlots=None, concatenationTimes=None, verbose=False):
